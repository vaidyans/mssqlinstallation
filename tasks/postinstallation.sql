...
USE [DBAMaint] -- create the database before executing this script in which the objects will be created.
...
BEGIN
  ...
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The table CommandLog is missing. Download .', 16, 1
  END
...
BEGIN
  ...
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The stored procedure CommandExecute is missing. Download.', 16, 1
  END
  ...
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The stored procedure CommandExecute needs to be updated. Download.', 16, 1
  END
  ...
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The table Queue is missing. Download .', 16, 1
  END
  ...
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The table QueueDatabase is missing. Download.', 16, 1
  END
  ...
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The documentation is available at.', 16, 1
  END
...
EXEC msdb.dbo.sp_add_jobschedule 
		...
		@freq_interval = 1,						-- Sunday
		@freq_recurrence_factor = 1,			                -- every week
		@active_start_time = 010300;				        -- 1:03 AM
...
EXEC msdb.dbo.sp_add_jobschedule 
		...
		@freq_interval = 1,			-- Sunday
		@freq_recurrence_factor = 1,		-- every week
		@active_start_time = 010400;		-- 1:04 AM
